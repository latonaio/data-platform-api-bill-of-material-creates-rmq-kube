...
import (
	"context"
	dpfm_api_caller "data-platform-api-bill-of-material-creates-rmq-kube/DPFM_API_Caller"
	...
	"data-platform-api-bill-of-material-creates-rmq-kube/existence_conf"
	"encoding/json"
	"fmt"
	"time"
...
func callProcess(rmq *rabbitmq.RabbitmqClient, caller *dpfm_api_caller.DPFMAPICaller, conf *config.Conf, msg rabbitmq.RabbitmqMessage) (err error) {
	...
	res, errs := caller.AsyncBillOfMaterialCreates(accepter, &input, &output, l)
	if len(errs) != 0 {
